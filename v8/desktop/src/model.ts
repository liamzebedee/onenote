...
function _findBlockFast(state: AppState, pageId: string, blockId: string): { block: Block; page: Page } | null {
  ...
  // Try the expected page first, fall back to global search
  const result = findPageInState(state, pageId);
  if (result) {
    const blk = result.page.blocks.find(b => b.id === blockId);
    if (blk) return { block: blk, page: result.page };
  }
  return _findBlockById(state, blockId);
}
...
function applyOp(state: AppState, op: Op): AppState {
  ...
  switch (op.type) {
    ...
    case 'block-text-op': {
      ...
      // Guard: don't let CRDT replay wipe existing HTML content.
      // Divergent CRDT state (e.g. snapshot from another device) can produce
      // empty text from non-empty html, causing permanent data loss.
      if (oldText !== newText && (newText || !blk.html)) {
...
function _finalizePages(pages: Page[]): void {
  for (const p of pages) {
    ...
    // Note: we no longer strip empty blocks here — doing so caused permanent
    // data loss when CRDT replay corrupted html to empty. The renderer already
    // handles empty blocks gracefully (auto-delete on blur for user-created ones).
