...
import type { TextDiff } from '../../core/src/types';

interface HtmlCharRange {
  start: number;
  end: number;
}
...
function getHtmlCharMap(html: string): HtmlCharRange[] {
  const map: HtmlCharRange[] = [];
  let i = 0;
  while (i < html.length) {
    if (html[i] === '<') {
...
function deleteHtmlChars(html: string, pos: number, count: number): string {
...
function insertHtmlText(html: string, pos: number, text: string): string {
  const map = getHtmlCharMap(html);
...
function applyDiffsToHtml(html: string, diffs: TextDiff[]): string {
  let result = html;
...
function applyTextChangeToHtml(html: string, oldText: string, newText: string): string {
...
export { applyDiffsToHtml, applyTextChangeToHtml };
