...
import type { CRDTId, CRDTOp, CRDTInsertOp, CRDTDeleteOp, CRDTSnapshot } from '../../core/src/types';
...
interface LinkedNode {
  ...
  _key: string;
  _next: LinkedNode | null;
  _prev: LinkedNode | null;
}
...
class TextCRDT {
  ...
  readonly ROOT_ID: CRDTId;
  _head: LinkedNode | null;
  _tail: LinkedNode | null;
  private _pendingDeletes: Set<string> | undefined;
  ...
  _compareIds(a: CRDTId, b: CRDTId): number {
  ...
  insertAt(parentId: CRDTId | null, char: string): CRDTInsertOp {
    if (!parentId) parentId = this.ROOT_ID;
    const id = this._nextId();
    const op: CRDTInsertOp = { type: 'insert', id, parentId, char };
    this.apply(op);
    return op;
  }

  insertTextAt(pos: number, text: string): CRDTInsertOp[] {
    const ops: CRDTInsertOp[] = [];
    let parentId: CRDTId | null = this._idAtVisiblePos(pos === 0 ? -1 : pos - 1);
  ...
  deleteAt(pos: number): CRDTDeleteOp | null {
    const id = this._idAtVisiblePos(pos);
    if (!id || this._key(id) === this._rootKey) return null;
    const op: CRDTDeleteOp = { type: 'delete', id };
    this.apply(op);
    return op;
  }

  _idAtVisiblePos(pos: number): CRDTId | null {
  ...
  private _insertAfter(after: LinkedNode, node: LinkedNode): void {
    node._prev = after;
  ...
  private _applyInsert(op: CRDTInsertOp): void {
    ...
    let insertAfter: LinkedNode = parent;
    while (cursor) {
  ...
  private _isDescendantOf(node: LinkedNode, ancestorKey: string): boolean {
    let current: LinkedNode | undefined = node;
  ...
  private _applyDelete(op: CRDTDeleteOp): void {
    const key = this._key(op.id);
  ...
  snapshot(): CRDTSnapshot {
    const nodes: CRDTSnapshot['nodes'] = [];
    let node = this._head;
    while (node) {
      nodes.push({
        id: node.id,
  ...
  static fromSnapshot(data: CRDTSnapshot): TextCRDT {
