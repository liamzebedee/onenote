import type {
  Page, Block, UndoDelta, MoveDelta, ResizeDelta, HtmlDelta,
  AddDelta, DeleteDelta, DeleteForwardDelta, ChecklistDelta, CropDelta,
} from './types.ts';

// Delta-based undo/redo — stores only what changed

interface UndoStack {
  past: UndoDelta[];
  future: UndoDelta[];
}

const stacks = new Map<string, UndoStack>();

function get(pageId: string): UndoStack {
  if (!stacks.has(pageId)) stacks.set(pageId, { past: [], future: [] });
  return stacks.get(pageId)!;
}

// Push a delta onto the undo stack.
// Deltas: { type: 'move', moves: [{ id, x, y }] }
//         { type: 'resize', id, w, (x, y)? }
//         { type: 'html', id, html }
//         { type: 'add', block }        — a block was added (undo = remove it)
//         { type: 'delete', blocks }    — blocks were deleted (undo = re-add them)
export function pushUndo(pageId: string, delta: UndoDelta): void {
  const s = get(pageId);
  s.past.push(delta);
  s.future = [];
  if (s.past.length > 200) s.past.shift();
}

export function applyUndo(pageId: string, page: Page): boolean {
  const s = get(pageId);
  if (!s.past.length) return false;
  const delta = s.past.pop()!;
  const reverse = apply(page, delta);
  if (reverse) s.future.push(reverse);
  return true;
}

export function applyRedo(pageId: string, page: Page): boolean {
  const s = get(pageId);
  if (!s.future.length) return false;
  const delta = s.future.pop()!;
  const reverse = apply(page, delta);
  if (reverse) s.past.push(reverse);
  return true;
}

// Apply a delta to page.blocks, return the reverse delta
function apply(page: Page, delta: UndoDelta): UndoDelta | null {
  const blocks = page.blocks;

  if (delta.type === 'move') {
    const rev: { id: string; x: number; y: number }[] = [];
    for (const m of delta.moves) {
      const b = blocks.find(b => b.id === m.id);
      if (b) {
        rev.push({ id: b.id, x: b.x, y: b.y });
        b.x = m.x;
        b.y = m.y;
      }
    }
    return { type: 'move', moves: rev };
  }

  if (delta.type === 'resize') {
    const b = blocks.find(b => b.id === delta.id);
    if (!b) return null;
    const rev: ResizeDelta = { type: 'resize', id: b.id, w: b.w };
    if (delta.x != null) { rev.x = b.x; rev.y = b.y; b.x = delta.x; b.y = delta.y!; }
    b.w = delta.w;
    return rev;
  }

  if (delta.type === 'html') {
    const b = blocks.find(b => b.id === delta.id);
    if (!b) return null;
    const rev: HtmlDelta = { type: 'html', id: b.id, html: b.html };
    b.html = delta.html;
    return rev;
  }

  if (delta.type === 'add') {
    // A block was added — undo by removing it
    const rev: DeleteDelta = { type: 'delete', blocks: [blocks.find(b => b.id === delta.id)].filter(Boolean) as Block[] };
    page.blocks = blocks.filter(b => b.id !== delta.id);
    return rev.blocks.length ? rev : null;
  }

  if (delta.type === 'delete') {
    // Blocks were deleted — undo by re-adding them
    const ids = delta.blocks.map(b => b.id);
    page.blocks = [...blocks, ...delta.blocks];
    return { type: 'deleteForward', ids } as DeleteForwardDelta;
  }

  if (delta.type === 'deleteForward') {
    // Redo of delete
    const removed = blocks.filter(b => delta.ids.includes(b.id));
    page.blocks = blocks.filter(b => !delta.ids.includes(b.id));
    return { type: 'delete', blocks: removed } as DeleteDelta;
  }

  if (delta.type === 'checklist') {
    const b = blocks.find(b => b.id === delta.id);
    if (!b) return null;
    const rev: ChecklistDelta = { type: 'checklist', id: b.id, items: (b.items || []).map(i => ({ ...i })) };
    b.items = delta.items;
    return rev;
  }

  if (delta.type === 'crop') {
    const b = blocks.find(b => b.id === delta.id);
    if (!b) return null;
    const rev: CropDelta = { type: 'crop', id: b.id, crop: b.crop ?? null };
    b.crop = delta.crop ?? undefined;
    return rev;
  }

  return null;
}
